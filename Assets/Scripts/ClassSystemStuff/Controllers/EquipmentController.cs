...
public class EquipmentController : MonoBehaviour
{
    ...
    private void Update()
    {
        if (_unit.GetUnitType != UnitTypes.Player) return;
    ...
    public bool[] SetEquippedWeapons()
    {
        ...
        for (int i = 0; i < equippedWeapons.Length; i++)
        {
            ...
            bool isValid = _unit.GetAllowedWeapons.Contains(equippedWeapons[i].GetWeaponType.ToString()) ||
                _unit.GetAllowedWeapons.Contains("All");
    ...
    public bool[] SetEquippedArmor()
    {
        ...
        for (int i = 0; i < equippedArmor.Length; i++)
        {
            ...
            bool isValid = _unit.GetAllowedArmor.Contains(equippedArmor[i].GetArmorType.ToString()) ||
                _unit.GetAllowedArmor.Contains("All");
            ...
            switch (type)
            {
                case DamageTypes.Physical:
                    totalDmg += (canUseMain ? _mainHand.GetPhysDamage : 0) +
                                (canUseOff ? _offHand.GetPhysDamage : 0) + _unit.GetStrength - 10; //subtract "baseline" strength stat from total strength to get modifier
                    break;
    ...
    public void DealWeaponDamage(UnitScript target)
    {
        ...
        if (exp != 0)
            _unit.ChangeExp(exp, exp > 0 ? true : false);
    }
    public IEnumerator SwingWeapons()
    {
        ...
        Vector3 forward = Vector3.zero; //PlayerMovement.instance.GetMouseDir;
        Debug.LogError("FIX WEAPON SWING FORWARD");
